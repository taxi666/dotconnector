...
Function DelNews(idstr)
	sqld = "select * from zhaopin where id="&idstr
  ...
  <tr> 
    <td valign="top" nowrap> 
      ...
      <table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolordark="#FFFFFF" bordercolorlight="#CCCCCC">
		...
		<form name="delnews" action="news_zp.asp?action=del" method=post>
        <%
...
%>
        <tr> 
          ...
          <td width="6%"> 
            <div align='center'><a href="addnews_zp.asp?newsid=<%=rs("id")%>" class="tt2">修改</a></div>
          </td>
        </tr>
        <%
		rs.movenext						
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class=buinput>
        <form name="pageform" method=get action="news_zp.asp">
          <tr>
            ...
            <td>
              <div align="right">跳到第 
                ...
                [<a href='news_zp.asp?pageno=<%=pageno-1%>' class="tt4">上一页</a>] 
                <%end if
			if cint(pageno)<cint(mpage) then
			%>
                [<a href='news_zp.asp?pageno=<%=pageno+1%>' class="tt4">下一页</a>] 
                <%end if%>
