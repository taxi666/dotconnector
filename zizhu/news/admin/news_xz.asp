...
Function DelNews(idstr)
	sqld = "select * from docs where id="&idstr
	set rsd=Server.CreateObject("ADODB.RecordSet")
	rsd.Open sqld,conn,1,3   
	if not rsd.EOF then
	
		fileurl = Trim(rsd("url1"))
       rsd.Delete
       rsd.Update
	 ...
	 Set fdel = CreateObject("Scripting.FileSystemObject")	
       ...
       if fileurl<>"" then
              tempFileDir = Server.MapPath("../../files/"&fileurl)
              If (fdel.FileExists(tempFileDir)) Then
	             fdel.DeleteFile(tempFileDir)
		      end if               
       end if
...
<table width="101%" border="0" align="center" cellspacing="3" cellpadding="3" bgcolor="#FFFFFF">
  <tr> 
    <td colspan="2"><%=homepage_title%> - <font color="#FF0000">文件下载管理</font></td>
  </tr>
  <tr> 
      ...
      <table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolordark="#FFFFFF" bordercolorlight="#CCCCCC">
        <tr bgcolor="#799AE1"> 
          ...
          <td width="44%" height="20" bgcolor="#799AE1"> 
            <div align="center"><font color="#FFFFFF">文件标题</font></div>
          </td>
		...
		<form name="delnews" action="news_xz.asp?action=del" method=post>
        <%
...
%>
        <tr> 
          ...
          <td width="6%"> 
            <div align='center'><a href="addnews_xz.asp?newsid=<%=rs("id")%>" class="tt2">修改</a></div>
          </td>
        </tr>
        <%
		rs.movenext						
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class=buinput>
        <form name="pageform" method=get action="news_xz.asp">
          <tr>
            ...
            <td>
              <div align="right">跳到第 
                ...
                [<a href='news_xz.asp?pageno=<%=pageno-1%>' class="tt4">上一页</a>] 
                <%end if
			if cint(pageno)<cint(mpage) then
			%>
                [<a href='news_xz.asp?pageno=<%=pageno+1%>' class="tt4">下一页</a>] 
                <%end if%>
