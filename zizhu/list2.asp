...
#div_mid {
	...
	width: 960px;
	margin:0px auto;
	background-image:url(images/midbg0.jpg);
}
...
#sidebar2,#sidebar3{clear:both;margin-top:4px}
.wenzi1{
	 height:40px;
	line-height:40px;
...
<div id="div_sidebar">
  ...
  exec="select * from zhaopin order by id desc"
  ...
  %>
    ...
    %>
      <li class="liclass1">
      <a href="view2.asp?id=<%=rs("id")%>" title="<%=rs("title")%>"  target="_bank"><%=title%></a></li>
      <li class="liclass2"><%=FormatDateTime(rs("date"),vbshortdate)%>
	 ...
	 if i>=MaxPerPage then exit do
  	...
  	if pageno<2 then
    		...
    		response.write "<a href='list2.asp?pageno=1'>首页</a>&nbsp;"
    		response.write "<a href='list2.asp?pageno="&(pageno-1)&"'>上一页</a>&nbsp;"
  	end if

  	if n-pageno<1 then
    		response.write "下一页 尾页"
  	else
    		response.write "<a href='list2.asp?pageno="&(pageno+1)&"'>"
    		response.write "下一页</a> <a href='list2.asp?pageno="&n&"'>尾页</a>"
  	end if
